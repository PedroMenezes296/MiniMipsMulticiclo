...
void Executar_Instrucao_M(int *reg_A, int *reg_B, int *estado_c, int *PC, struct instrucao *RI, struct ULA_saida *saida) {   //Executa as instru��es
    ...
    if(RI->opcode == -1) {                                            // se o opcode estiver como -1, indica que n�o h� mais instru��es v�lidas
            printf("\tPC esta apontando para um campo vazio!\n\n");
    }
    else if(RI->opcode != 8){
        int resultado = Calculos_ULA_M(reg_A, reg_B, estado_c, PC, RI, saida);    // chama a fun��o da ula que verifica o opcode e executa a opera��o
...
void Ciclo(int *reg_dado, struct ULA_saida *saida, int *reg_A, int *reg_B, int *estado_c, int *PC, struct instrucao *RI, struct instrucao *inst_name,  int *banco_de_registradores){
    ...
    if(*estado_c == 6){
		*reg_B = saida->reg_ULA;
		Escrever_Registrador(banco_de_registradores, RI->rt , *reg_B);
	}
    if(*estado_c == 7){
        saida->reg_ULA = Calculos_ULA_M(reg_A, reg_B, estado_c, PC, RI,saida);
    }
    ...
    switch(*estado_c) {
        ...
        case 1:
         if(RI->opcode == 0) {
                ...
                *estado_c = 2;
            } else if (RI->opcode == 2){ // op code do jump 
				*estado_c = 10;
			} else if (RI->opcode == 8){
				*estado_c = 9;
			}
